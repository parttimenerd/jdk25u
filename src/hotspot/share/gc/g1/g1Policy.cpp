...
void G1Policy::record_concurrent_refinement_stats(size_t pending_cards,
  ...
  {
    EventG1ConcurrentRefinement event;
    if (event.should_commit()) {
      event.set_gcId(GCId::current());
      event.set_refinementTime(total_stats.refinement_time());
      event.set_refinedCards(total_stats.refined_cards());
      event.set_precleanedCards(total_stats.precleaned_cards());
      event.set_dirtiedCards(total_stats.dirtied_cards());
      event.commit();
    }
  }
